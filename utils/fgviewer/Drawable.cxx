...
{
public:
    _ResizedCallback(Drawable* drawable) : _drawable(drawable)
    {
    }
...
Drawable::Drawable(const std::string& name) : _name(name),
                                              _position(0, 0),
                                              _size(600, 800),
                                              _offscreen(false),
                                              _fullscreen(false)
...
void Drawable::attachSlaveCamera(SlaveCamera* slaveCamera)
{
    if (!slaveCamera)
        return;
...
bool Drawable::resize(int x, int y, int width, int height)
{
...
bool Drawable::realize(Viewer& viewer)
{
...
osg::GraphicsContext* Drawable::_realizeImplementation(Viewer& viewer)
{
    osg::ref_ptr<osg::GraphicsContext::Traits> traits = _getTraits(viewer);
    return viewer.createGraphicsContext(traits.get());
}

osg::ref_ptr<osg::GraphicsContext::Traits> Drawable::_getTraits(Viewer& viewer)
{
